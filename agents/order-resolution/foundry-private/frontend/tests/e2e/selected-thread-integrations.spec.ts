...
test("optional CopilotKit failure leaves the native timeline available", async ({
  page,
}) => {
  await page.route("**/api/copilotkit", async (route) => {
    await route.fulfill({
      status: 404,
      contentType: "application/json",
      body: JSON.stringify({ detail: "CopilotKit is not deployed" }),
    });
  });
  ...
  await expect(page.getByTestId("copilot-thread-assistant").getByRole("status")).toContainText(
    "The optional CopilotKit endpoint returned 404.",
  );
  await expect(page.locator(".panel-timeline")).toContainText("workflow.output");
});
...
test("CopilotKit bridge posts selected-thread AG-UI input to the root endpoint", async ({
  ...
  expect(requests[0]).not.toHaveProperty("events");
  expect(JSON.stringify(requests[0])).not.toContain("Resolution complete");
