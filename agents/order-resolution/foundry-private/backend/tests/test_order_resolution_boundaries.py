    ...
    events = [
        ...
        WorkflowEvent(
            ...
            payload={
                ...
                "mcp_result": {"authorization": "private-token"},
            },
        ),
        WorkflowEvent(
            type=event_types.HITL_REQUEST,
...
@pytest.mark.asyncio
async def test_selected_thread_streams_are_durable_and_redacted(
    ...
    repository = SelectedThreadRepository()
    monkeypatch.setattr(copilotkit_router, "workflow_run_repository", repository)
    monkeypatch.setattr(chat_router, "workflow_run_repository", repository)

    copilotkit_response = await copilotkit_router.stream_selected_thread(
        CopilotKitBridgeRequest(
            threadId="thread-123",
            messages=[{"role": "user", "content": "raw workflow prompt"}],
        )
    )
    agui_response = await chat_router.stream_chat_agui("thread-123")
    streams = [copilotkit_response.body_iterator, agui_response.body_iterator]
    try:
        chunks = []
        for stream in streams:
            chunks.extend([await stream.__anext__() for _ in range(3)])
    finally:
        for stream in streams:
            await stream.aclose()

    serialized = b"".join(
        item if isinstance(item, bytes) else item.encode() for item in chunks
    ).decode()
    assert serialized.count('"type":"RUN_STARTED"') == 2
    assert serialized.count('"type":"TEXT_MESSAGE_CONTENT"') == 2
    assert serialized.count("The resolution workflow completed.") == 2
...
def test_selected_thread_routes_reject_bad_and_unknown_thread_ids(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class NoSelectedThreadRepository:
        def __init__(self) -> None:
            self.selected_thread_lookups: list[str] = []

        def get_workflow_run(self, thread_id: str) -> None:
            self.selected_thread_lookups.append(thread_id)
            return None
        ...
        def list_workflow_events(
            ...
            pytest.fail("Rejected selected-thread requests must not read workflow events.")

    repository = NoSelectedThreadRepository()
    monkeypatch.setattr(chat_router, "workflow_run_repository", repository)
    monkeypatch.setattr(copilotkit_router, "workflow_run_repository", repository)
    client = TestClient(app)

    responses = [
        client.get("/api/chat/stream/bad%20thread/ag-ui"),
        client.get("/api/chat/stream/unknown-thread/ag-ui"),
        client.post("/api/copilotkit", json={"threadId": "bad thread"}),
        client.post("/api/copilotkit", json={"threadId": "unknown-thread"}),
    ]

    assert [response.status_code for response in responses] == [404, 404, 422, 404]
    assert repository.selected_thread_lookups == ["unknown-thread", "unknown-thread"]

    serialized = "\n".join(response.text for response in responses)
    assert "Selected workflow thread was not found." in serialized
    assert "workflow.stage" not in serialized
    assert "ORD-9000" not in serialized
    assert "private-token" not in serialized
...
def test_copilotkit_bridge_request_accepts_standard_input_without_exposing_it() -> None:
    request = CopilotKitBridgeRequest(
        ...
        runId="run-123",
        messages=[{"role": "user", "content": "raw workflow prompt"}],
        state={"credential": "private-token"},
