...
class FakeResponsesWorkflow:
    ...
    async def start_workflow(self, *, thread_id: str, message: str) -> str:
        self.started.append({"thread_id": thread_id, "message": message})
        return thread_id
...
async def test_order_resolution_service_delegates_to_responses_without_second_workflow() -> None:
    ...
    assert responses.started == [
        {"thread_id": "thread-123", "message": "Order ORD-1001 arrived late."}
    ]
    ...
    events = [
        ...
        WorkflowEvent(
            ...
            payload={
                ...
                "mcp_result": {"authorization": "Bearer private-token"},
            },
        ),
        WorkflowEvent(
            type=event_types.HITL_REQUEST,
        ...
        WorkflowEvent(
            ...
            payload={
                ...
                "status": "completed",
                "submission_id": "Bearer private-token",
            },
        ),
    ]
...
def test_copilotkit_bridge_request_accepts_standard_input_without_exposing_it() -> None:
    request = CopilotKitBridgeRequest(
        ...
        runId="run-123",
        messages=[{"role": "user", "content": "raw workflow prompt"}],
        state={"credential": "Bearer private-token"},
