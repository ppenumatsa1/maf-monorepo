    ...
    async def run(self, request: CheckRequest, ctx: WorkflowContext[CheckResult]) -> None:
        ...
        async def external_call() -> dict:
            score = max(
                ...
                min(
                    1.0,
                    (request.application.age / 100.0)
                    + (request.application.requested_coverage / 1_000_000.0),
                ),
            )
            return {"score": round(score, 3), "details": {"model": "deterministic-risk-v1"}}

        with workflow_stage_span("stage.risk_check", workflow_attributes(request, self.id)):
            invocation = await invoke_check_operation(
                repository=self.repository,
                settings=self.settings,
                workflow_run_id=request.workflow_run_id,
                application_id=request.application.application_id,
                check_type=CheckType.RISK,
        ...
        self.repository.save_underwriting_result(
            ...
            CheckType.RISK.value,
            result.to_dict(),
            invocation.idempotency_key,
        )
        self.repository.upsert_idempotency(
            invocation.idempotency_key, "risk_check", "completed", result.to_dict()
        )
