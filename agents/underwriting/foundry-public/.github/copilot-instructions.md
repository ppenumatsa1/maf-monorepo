...
- Keep one underwriting workflow path implemented in MAF parent and child workflows; do not add a shadow orchestrator, alternate workflow engine, or compatibility shim.
...
- Preserve fan-out/fan-in semantics: each child workflow emits one check result, `fan_in_aggregator` merges incrementally, and `final_decision` remains deterministic before rationale generation.
- Preserve checkpoint and resume behavior keyed by `workflow_run_id`; replay and resume must remain idempotent and observable.
...
- `make foundry-smoke`
- `make foundry-eval` (`make foundry-trace-eval` is the same report-only gate)
