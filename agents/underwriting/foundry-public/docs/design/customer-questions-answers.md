...
- Parent underwriting workflow:  
  https://github.com/ppenumatsa1/maf-underwriting-agent/blob/main/backend/app/maf/workflows/parent_underwriting_workflow.py
- Fan-in aggregator:  
...
flowchart TD
    ...
    B --> C1[Risk Check Subworkflow]
    B --> C2[Credit Check Subworkflow]
    B --> C3[Medical Check Subworkflow]
    B --> C4[Driving Check Subworkflow]
...
flowchart LR
    ...
    RUNNER --> WF[Parent Workflow + Child Workflows]
...
### 3.1 Workflow checkpointing, subworkflows, and recovery

- Resume is **parent-run driven** by `workflow_run_id`; the app loads the latest checkpoint for that run and resumes execution.  
  Docs: https://learn.microsoft.com/en-us/agent-framework/workflows/checkpoints
- Child workflow progress is restored as part of checkpointed workflow state for that run; operationally, this prototype exposes parent-run resume (`make resume RUN_ID=...`).
...
- each child emits independently
...
We do not claim `AddFanInBarrierEdge` always delivers one-message-at-a-time unless that specific API behavior is validated directly. This prototype validates incremental aggregation via explicit child-to-aggregator edges.

Prototype parent workflow:  
https://github.com/ppenumatsa1/maf-underwriting-agent/blob/main/backend/app/maf/workflows/parent_underwriting_workflow.py
...
Use shared workflow state for aggregation (`expected_checks`, `completed_checks`, `child_results`, `final_decision_emitted`) and trigger final decision only when all expected checks are complete.
...
2. Child check requests carry run/application identity.
3. Child results are sent as typed messages.
