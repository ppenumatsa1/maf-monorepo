...
azd_environment="${AZD_ENV_NAME:-underwriting-foundry-public}"

az account set --subscription "$subscription_id"
azd auth login --check-status >/dev/null
location="$(az group show --name "$resource_group" --query location -o tsv)"
# This is deliberately pinned to the discovered server location. It must not
# query the server: bootstrap runs between deletion and declarative recreation.
...
(
  cd "$FOUNDRY_DIR"
  AZURE_DEV_USER_AGENT=microsoft_foundry_skill azd env select "$azd_environment" \
    || AZURE_DEV_USER_AGENT=microsoft_foundry_skill azd env new "$azd_environment"
  ...
  set_value LOG_ANALYTICS_WORKSPACE_NAME "${LOG_ANALYTICS_WORKSPACE_NAME:-azlawhcedyxchnbtm}"
  set_value HOSTED_AGENT_NAME "underwriting-hosted"
  set_value FOUNDRY_MODEL_DEPLOYMENT_NAME "${FOUNDRY_MODEL_DEPLOYMENT_NAME:-underwriting-gpt-4-1-mini}"
  set_value AZURE_OPENAI_ENDPOINT "$(az cognitiveservices account show --resource-group "$resource_group" --name "$foundry_account" --query properties.endpoint -o tsv)"
  set_value APPLICATIONINSIGHTS_CONNECTION_STRING "$(az monitor app-insights component show --resource-group "$resource_group" --app "$app_insights" --query connectionString -o tsv)"
)

echo "Configured azd environment '$azd_environment' for existing underwriting resources."
echo "Run make foundry-provision, then make foundry-postgres-credentials with secure credentials before deployment."
