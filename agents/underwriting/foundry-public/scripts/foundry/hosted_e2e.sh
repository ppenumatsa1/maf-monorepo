...
write_evidence() {
  ...
  jq -n \
    ...
    '{
      ...
      workflow_run_ids: [$happy_run_id, $recovered_run_id],
...
backend_fqdn="${BACKEND_FQDN:-$(az containerapp show --resource-group "$resource_group" --name "$backend_name" --query 'properties.configuration.ingress.fqdn' --output tsv)}"
frontend_fqdn="${FRONTEND_FQDN:-$(az containerapp show --resource-group "$resource_group" --name "$frontend_name" --query 'properties.configuration.ingress.fqdn' --output tsv)}"
backend_url="${BACKEND_URL:-https://${backend_fqdn}}"
...
api_get_json "$backend_url/health" >/dev/null
curl --fail --silent --show-error --max-time 60 "$frontend_url" >/dev/null

happy_response="$(
  api_post_json \
    "$backend_url/api/v1/underwriting/runs" \
...
happy_events="$(api_get_json "$backend_url/api/v1/underwriting/runs/$happy_run_id/events")"
...
crash_response="$(
  api_post_json \
    "$backend_url/api/v1/underwriting/runs" \
    "$(build_start_payload "$recovered_run_id" "app-hosted-recover-${run_nonce}" true "medical_check")"
)"
assert_json_field "$crash_response" '.status == "CRASHED"'

crash_checkpoints="$(api_get_json "$backend_url/api/v1/underwriting/runs/$recovered_run_id/checkpoints")"
assert_json_field "$crash_checkpoints" 'length > 0'

resume_response="$(api_post_json "$backend_url/api/v1/underwriting/runs/$recovered_run_id/resume" '{}')"
...
recovered_events="$(api_get_json "$backend_url/api/v1/underwriting/runs/$recovered_run_id/events")"
assert_json_field "$recovered_events" 'length > 0'
assert_json_field "$recovered_events" 'map(.created_at) == (map(.created_at) | sort)'
assert_json_field "$recovered_events" 'map(.event_type) | index("retry_attempt") != null'
assert_json_field "$recovered_events" 'map(select(.event_type == "fan_in_result_received")) | length == 4'

recovered_run="$(api_get_json "$backend_url/api/v1/underwriting/runs/$recovered_run_id")"
...
echo "Hosted underwriting E2E passed for ${happy_run_id} and ${recovered_run_id}."
